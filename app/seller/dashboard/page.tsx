...
export default async function SellerDashboard() {
  ...
  if (!user || (user as any).role !== 'seller') redirect('/seller/register')

  const [listings, orders] = await Promise.all([
    sql`SELECT * FROM listings WHERE seller_id = ${user.id!} ORDER BY created_at DESC`,
    sql`SELECT * FROM orders WHERE seller_id = ${user.id!} ORDER BY created_at DESC`,
  ])

  const totalRevenue = orders
    .filter((o: any) => o.status !== 'cancelled' && o.status !== 'pending_payment')
    .reduce((sum: number, o: any) => sum + o.amount - o.commission_amount, 0)
    ...
    <main className="bg-gray-50 min-h-screen" dir="rtl">
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-6 py-6 flex items-center justify-between">
          ...
          <Link href="/seller/listings/new" className="btn-primary">+ מוצר חדש</Link>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            { label: 'סה"כ הזמנות',   value: orders.length, icon: '🛒', color: 'text-green-600' },
        ...
        <Link href="/seller/b2b" className="card p-5 flex items-center gap-4 hover:shadow-card-hover transition-shadow">
          <div className="w-12 h-12 bg-gray-50 rounded-xl flex items-center justify-center text-2xl shrink-0">📋</div>
          <div>
            <p className="font-bold text-navy-900">בקשות הצעת מחיר</p>
            <p className="text-xs text-gray-400 mt-0.5">הצעות B2B שהתקבלו</p>
          </div>
          <span className="mr-auto text-brand-600 text-xl">←</span>
        </Link>

        <div className="card overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 className="font-bold text-navy-900">המוצרים שלי</h2>
            <Link href="/seller/listings/new" className="text-sm text-brand-600 hover:underline font-medium">+ הוסף מוצר</Link>
          </div>
              ...
              {listings.map((l: any) => (
                <div key={l.id} className="px-6 py-4 flex items-center gap-4 hover:bg-gray-50 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-navy-900 truncate">{TYPE_LABEL[l.type] ?? l.type}{l.title ? ` — ${l.title}` : ''}</p>
                    <p className="text-xs text-gray-400 mt-0.5">{l.location ?? 'ללא מיקום'}</p>
                  </div>
                  <p className="font-bold text-brand-600 shrink-0">₪{(l.price / 100).toLocaleString('he-IL')}</p>
                  <span className={STATUS_BADGE[l.status] ?? 'badge bg-gray-100 text-gray-600'}>{STATUS_LABEL[l.status] ?? l.status}</span>
                  <Link href={`/seller/listings/${l.id}/edit`} className="text-sm text-brand-600 hover:text-brand-800 font-medium shrink-0">עריכה</Link>
                </div>
              ))}
            </div>
          )}
