...
export default async function SellerB2BList() {
  ...
  const rows = await sql`
    SELECT r.id, r.shelter_type, r.quantity, r.location, r.target_date, r.status,
           rs.status AS invite_status, 'shortlist' AS source
    FROM b2b_request_sellers rs JOIN b2b_requests r ON r.id = rs.request_id
    WHERE rs.seller_id = ${user.id!}
    UNION
    SELECT r.id, r.shelter_type, r.quantity, r.location, r.target_date, r.status,
           NULL AS invite_status, 'listing' AS source
    FROM b2b_requests r
    WHERE r.seller_id = ${user.id!}
        ...
        <div className="space-y-3">
          {rows.map((r: any) => (
            <Link key={r.id} href={`/seller/b2b/${r.id}`} className="card p-4 flex justify-between hover:shadow-card-hover">
              ...
              <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-brand-100 text-brand-700">{r.invite_status ?? r.status}</span>
            </Link>
          ))}
        </div>
      )}
